pipeline {
    ...
    stages {
        stage('Git Checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/heyanoop/argocd-sample'
            }
        }
        ...
        stage('Modify image') {
            steps {
                script {
                    sh "sed -i 's/tag: .*/tag: '${DOCKER_TAG}'/' argo-helm/values.yaml"
                }
            }
        }
        ...
        stage('Package Helm Chart') {
            steps {
                script {
                    ...
                    sh "helm package argo-helm --version ${chartVersion} -d ."
                }
            }
        }
